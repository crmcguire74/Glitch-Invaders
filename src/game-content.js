...
export const CHAPTERS = [
  { key:'invaders', name:'VOID INVADERS', cabinet:'STAR LEGION', legacy:'FORMATION ASSAULT', texture:'/assets/world-space-v1.png', color:0x63efff, accent:0xff3dba, mechanic:'formation', rule:'Break the airborne alien fleet before it descends through the arcade.', objective:'DESTROY THE AIRBORNE FORMATION' },
...
function fighter(color) {
  const g=new THREE.Group();const cloth=physical(color,0x120408,.18,.5,.18),skin=new THREE.MeshStandardMaterial({color:0xb97956,roughness:.7}),hair=new THREE.MeshStandardMaterial({color:0x17100d,roughness:.9}),wrap=physical(0x2b1011,0xff2d4f,.35,.55,.2);
  const torso=mesh(new THREE.CapsuleGeometry(.38,.82,10,20),cloth,g,[0,1.03,0],[0,0,0],[1.12,1,1]);mesh(new THREE.BoxGeometry(.12,.78,.055),MAT.bone,torso,[-.14,.02,-.36],[0,0,.32]);mesh(new THREE.BoxGeometry(.12,.78,.055),MAT.bone,torso,[.14,.02,-.36],[0,0,-.32]);mesh(new THREE.TorusGeometry(.38,.055,10,28,Math.PI),wrap,g,[0,.76,-.02],[Math.PI/2,0,0]);
  const head=mesh(new THREE.SphereGeometry(.3,28,20),skin,g,[0,1.83,-.02],[0,0,0],[.92,1.08,.88]);mesh(new THREE.SphereGeometry(.31,22,14,0,Math.PI*2,0,Math.PI/2),hair,head,[0,.08,.02],[0,0,0],[1,1,.96]);for(const x of [-.1,.1])mesh(new THREE.SphereGeometry(.028,12,8),glow(0xffcf78,1.5),head,[x,.03,-.27]);
  for(const side of [-1,1]){mesh(new THREE.SphereGeometry(.22,18,12),cloth,g,[side*.39,1.38,0],[0,0,0],[1.1,.75,1]);tube([new THREE.Vector3(side*.34,1.34,0),new THREE.Vector3(side*.62,1.02,-.2),new THREE.Vector3(side*.76,1.34,-.48)],.12,skin,g);tube([new THREE.Vector3(side*.2,.68,0),new THREE.Vector3(side*.34,.26,.12),new THREE.Vector3(side*.46,-.17,-.14)],.145,cloth,g);mesh(new THREE.SphereGeometry(.18,18,12),wrap,g,[side*.78,1.34,-.5]);mesh(new RoundedBoxGeometry(.36,.18,.62,4,.05),MAT.rubber,g,[side*.5,-.2,-.3]);}
  return g;
}
...
export function createEnvironment(chapter, texture, renderer) {
  ...
  for(const side of [-1,1])for(let z=-10;z<=8;z+=2.25)mesh(new RoundedBoxGeometry(.1,.045,.7,3,.02),glow(chapter.color,.8),g,[side*9.65,.025,z]);
