...
const textureLoader=new THREE.TextureLoader(),textures=new Array(CHAPTERS.length);function worldTexture(index){if(!textures[index])textures[index]=textureLoader.load(CHAPTERS[index].texture,()=>{if(automatedCapture)render();});return textures[index];}
...
function buildArcade(){
  ...
  const ceilingMat=physical(0x0a0c12,0,0,.5,.9);for(let z=-12;z<11;z+=3.8){mesh(new THREE.BoxGeometry(27,.16,.22),ceilingMat,arcadeRoot,[0,9.55,z]);for(const x of [-7,0,7])mesh(new THREE.CylinderGeometry(.045,.045,2.7,10),glow(x===0?0xff4baf:0x5cf5ff,1.8),arcadeRoot,[x,9.38,z],[0,0,Math.PI/2]);}
...
const entities=[],shots=[],enemyShots=[],effects=[],pickups=[];const fastTest=automatedCapture&&params.get('fast')==='1';let currentEnvironment=null,helpReturnMode='playing';
...
function levelQuota(chapter=state.chapter,level=state.level){const key=CHAPTERS[chapter].key,value={invaders:8+level*3,asteroids:7+level*3,crossing:1+level,centipede:level,tapper:7+level*3,dig:7+level*3,jungle:1+level,brawler:1,cube:7+level*3}[key]||8;return fastTest?Math.min(value,key==='crossing'||key==='jungle'?1:4):value;}
...
function resetGame(){
  ...
  cabinets.forEach((c,i)=>{c.visible=!automatedCapture||i===requestedChapter||i===(requestedChapter+1)%CHAPTERS.length||i===(requestedChapter+CHAPTERS.length-1)%CHAPTERS.length;});
...
function beginTakeover(chapter,first=false){
  ...
  cabinets.forEach((c,i)=>c.userData.active=i===chapter);const ch=CHAPTERS[chapter];showWorldCard(`CABINET ${String(chapter+1).padStart(2,'0')} // REALITY TAKEOVER`,ch.name,ch.rule);ui.transmission.classList.add('hidden');state.messageTimer=0;
...
function installWorld(index){
  clearGroup(worldRoot);const ch=CHAPTERS[index];currentEnvironment=createEnvironment(ch,worldTexture(index),renderer);worldRoot.add(currentEnvironment);currentEnvironment.userData.skyMat.opacity=automatedCapture?1:0;state.environmentInstalled=true;
...
function updateTransition(dt){
  state.transitionTime+=dt*(fastTest?8:1);const t=state.transitionTime,c=cabinets[state.pendingChapter];c.userData.portal.material.opacity=Math.min(.92,t*.75);c.userData.portal.scale.setScalar(1+Math.sin(t*8)*.08+t*.08);c.userData.light.intensity=18+Math.sin(t*12)*9;
...
function seedLevel(){
  const key=CHAPTERS[state.chapter].key;if(key==='invaders'){const cols=fastTest?2:4+state.level,rows=2;for(let row=0;row<rows;row++)for(let col=0;col<cols;col++)spawnEntity('invader',(col-(cols-1)/2)*2.8,-5.8+row*2.05,{hp:1+Math.floor(state.level/3),vx:1.25*difficulty(),scale:1.35});}
...
function spawnLoop(){
  ...
  if(key==='crossing'){const lane=Math.floor(Math.random()*5),z=5-lane*2.7,left=Math.random()>.5;spawnEntity('car',left?-11:11,z,{color:left?0xe74338:0x1e92b6,vx:(left?1:-1)*(4.5+lane*.7)*d,lane,minor:true});}
  ...
  if(key==='jungle'){const left=Math.random()>.5;spawnEntity('beast',left?-10:10,-7+Math.random()*12,{color:Math.random()>.5?0x7a9b2e:0x945f31,vx:(left?1:-1)*3.2*d,hp:2+state.level,minor:true});}
  if(key==='cube')spawnEntity('hopper',(Math.random()-.5)*12,-8,{color:Math.random()>.5?0x9a55d2:0x4ec9bb,vz:1.6*d,hp:1+state.level,minor:true});
...
function shoot(origin=null,direction=null){if(state.mode!=='playing'||state.shotCooldown>0)return;state.shotCooldown=.13;const dir=direction?direction.clone().normalize():new THREE.Vector3(-Math.sin(state.player.bodyYaw),0,-Math.cos(state.player.bodyYaw));if(Math.abs(dir.y)>.65)dir.y=0;dir.normalize();const start=origin?origin.clone():new THREE.Vector3(state.player.x,.58,state.player.z).addScaledVector(dir,.78),shotSpeed=fastTest?72:19;const m=mesh(new THREE.CapsuleGeometry(.065,.54,5,10),new THREE.MeshBasicMaterial({color:CHAPTERS[state.chapter].color,blending:THREE.AdditiveBlending}),projectileRoot,start.toArray());m.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir);shots.push({mesh:m,x:start.x,y:start.y,z:start.z,vx:dir.x*shotSpeed,vy:dir.y*shotSpeed,vz:dir.z*shotSpeed,life:1.65,radius:.17});tone(620,.045,'square',.018,260);if(state.tutorialActive&&state.tutorialStage===1){state.tutorialStage=2;completeTutorial();}}
...
function updateCabinets(dt){cabinets.forEach((c,i)=>{const active=i===state.chapter||i===state.pendingChapter;c.userData.screenMat.emissiveIntensity=THREE.MathUtils.damp(c.userData.screenMat.emissiveIntensity,active?1.35:.55,4,dt);c.userData.light.intensity=THREE.MathUtils.damp(c.userData.light.intensity,active?5:1.25,4,dt);c.position.y=c.userData.baseY+(active?Math.sin(state.time*2.4+i)*.025:0);c.rotation.z=active?Math.sin(state.time*3+i)*.003:0;});}
...
function render(){if(!renderer.xr.isPresenting){const shake=state.shake*.12;if(state.mode==='title'){camera.position.set(2.8+Math.sin(state.time*.18)*.35,3.35,11.4);camera.lookAt(9.3,1.9,4.4);worldLight.position.set(8,4,4);worldLight.intensity=48;}else if(state.mode==='transition'){const c=cabinets[state.pendingChapter],side=Math.sign(c.position.x)||1;camera.position.set(-side*2.4+(Math.random()-.5)*shake,4.1+(Math.random()-.5)*shake,11.8);camera.lookAt(c.position.x*.83,1.9,c.position.z-.7);worldLight.position.set(c.position.x*.7,4,c.position.z);worldLight.intensity=62;}else{camera.position.set(desktopCamera.position.x+(Math.random()-.5)*shake,desktopCamera.position.y+(Math.random()-.5)*shake,desktopCamera.position.z+(Math.random()-.5)*shake);camera.lookAt(desktopCamera.target);worldLight.position.set(0,6,-2);}if(automatedCapture)renderer.render(scene,camera);else composer.render();}else renderer.render(scene,camera);}
